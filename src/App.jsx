...
// ── Color palette (Altimate-inspired: black, white, monospace-accented) ──
const C = {
  bg:'#000000', sb:'#050505', panel:'#0A0A0A',
  b1:'#1A1A1A', b2:'#262626',
  text:'#FFFFFF', muted:'#8A8A8A', dim:'#4A4A4A',
  acc:'#FFFFFF', onAcc:'#000000', grad:'#FFFFFF',
  mono:"'JetBrains Mono',monospace"
}
const eyebrow = { fontSize:10.5, fontWeight:600, letterSpacing:'0.14em', textTransform:'uppercase', color:C.muted }
const btn = (active, danger) => ({
  padding:'6px 13px', borderRadius:3, border:`1px solid ${active?C.acc:danger?'#3A1010':C.b2}`, cursor:'pointer', fontSize:12.5, fontWeight:500,
  background: danger ? '#160A0A' : active ? C.acc : 'transparent',
  color:       danger ? '#F87171' : active ? C.onAcc : C.muted,
  transition:'all .15s'
})
...
function MsgContent({ text = '' }) {
  return (
    <>{text.split(/(```[\s\S]*?```)/g).map((p,i) => {
      ...
      return (
        <pre key={i} style={{background:'#0D1117',border:'1px solid #30363D',borderRadius:4,padding:'12px 14px',
  ...
  return (
    <div style={{marginBottom:8}}>
      <button onClick={() => setOpen(x=>!x)}
        style={{display:'flex',alignItems:'center',gap:6,padding:'5px 11px',background:'#0A0A0A',border:'1px solid #333333',borderRadius:4,cursor:'pointer',fontSize:12,color:'#E5E5E5',marginBottom:open?6:0}}>
        {open?'▾':'▸'} 💭 Claude's reasoning {!open && <span style={{color:'#666666'}}>(expand)</span>}
      </button>
      {open && (
        <div style={{padding:'12px 14px',background:'#0A0A0A',border:'1px solid #262626',borderRadius:5,
          fontSize:12.5,color:'#999999',fontFamily:"'JetBrains Mono',monospace",lineHeight:1.75,
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{minHeight:'100vh',background:C.bg,display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}}>
      {/* faint grid backdrop for depth, altimate-style */}
      <div style={{position:'absolute',inset:0,backgroundImage:'linear-gradient(#FFFFFF08 1px,transparent 1px),linear-gradient(90deg,#FFFFFF08 1px,transparent 1px)',backgroundSize:'42px 42px',maskImage:'radial-gradient(ellipse 60% 50% at 50% 40%,black,transparent)'}}/>

      <div style={{textAlign:'center',maxWidth:440,padding:32,width:'100%',position:'relative'}}>
        <div style={{...eyebrow,display:'inline-flex',alignItems:'center',gap:8,marginBottom:22}}>
          <span>◈</span> Shared AI Workspace <span>⬡</span>
        </div>

        <h1 style={{color:C.text,fontSize:38,fontWeight:700,margin:'0 0 10px',letterSpacing:-1.2,fontFamily:'Inter,system-ui,sans-serif'}}>VaultAI</h1>
        <p style={{color:C.muted,fontSize:14.5,margin:'0 0 26px',lineHeight:1.6}}>Real-time project notebooks, chat, and files —<br/>synced live across every teammate.</p>

        <div style={{display:'inline-flex',alignItems:'center',gap:7,background:C.panel,border:`1px solid ${C.b2}`,borderRadius:4,padding:'5px 14px',marginBottom:26}}>
          <div style={{width:6,height:6,borderRadius:'50%',background:'#22C55E',boxShadow:'0 0 6px #22C55E'}}/>
          <span style={{color:C.muted,fontSize:11.5,fontFamily:C.mono,letterSpacing:0.2}}>AI via Claude Desktop MCP · no API key needed</span>
        </div>

        <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:16}}>
          <input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()}
            placeholder="Your name" autoFocus
            style={{width:'100%',padding:'13px 14px',background:C.panel,border:`1px solid ${C.b2}`,borderRadius:4,color:C.text,fontSize:14,outline:'none',fontFamily:'Inter,system-ui,sans-serif'}}/>
          {err && <p style={{color:'#F87171',fontSize:13,margin:0,textAlign:'left'}}>{err}</p>}
        </div>
        <button onClick={submit}
          style={{width:'100%',padding:'13px',background:C.grad,border:'none',borderRadius:4,color:C.onAcc,fontSize:15,fontWeight:600,cursor:'pointer',marginBottom:22,letterSpacing:0.2}}>
          Enter Workspace →
        </button>

        <div style={{...eyebrow,marginBottom:8,fontSize:9.5}}>Get started</div>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,background:'#050505',border:`1px solid ${C.b1}`,borderRadius:4,padding:'10px 14px',marginBottom:20,textAlign:'left'}}>
          <span style={{fontFamily:C.mono,fontSize:12.5,color:'#E5E5E5'}}><span style={{color:C.dim}}>$</span> Open the link your teammate shared with you</span>
        </div>

        <button onClick={() => setGuide(true)}
          style={{background:'none',border:'none',color:C.muted,fontSize:12.5,cursor:'pointer',letterSpacing:0.2}}>
          Setup &amp; usage guide →
        </button>
        <p style={{color:'#333333',fontSize:11,marginTop:18,fontFamily:C.mono}}>Free · Claude responds via MCP · no API key required</p>
      </div>
      ...
      <div key={path}>
        ...
        <div
          ...
          style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 10px',
            paddingLeft: 10 + indent, cursor:'pointer', borderRadius:4,
        ...
        >
          ...
          <span style={{color:'#E5E5E5',fontSize:12.5,fontWeight:600,fontFamily:"'JetBrains Mono',monospace"}}>{name}/</span>
            ...
            {files.map(f => (
              ...
              >
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{color:'#EDEDED',fontSize:12.5,fontWeight:500,
                  ...
                  {isText(f.name) && (
                    <button onClick={()=>onPreview(f)}
                      style={{padding:'3px 8px',background:'#0A1929',border:'1px solid #1E3A5F',
                        borderRadius:3,color:'#60A5FA',fontSize:11,cursor:'pointer'}}>👁</button>
                  )}
                  <button onClick={()=>onDownload(f)}
                    style={{padding:'3px 8px',background:'#0A1929',border:'1px solid #1E3A5F',
                      borderRadius:3,color:'#38BDF8',fontSize:11,cursor:'pointer'}}>↓</button>
                  <button onClick={()=>onDelete(f)}
                    style={{padding:'3px 7px',background:'#160A0A',border:'1px solid #3A1010',
                      borderRadius:3,color:'#F87171',fontSize:11,cursor:'pointer'}}>✕</button>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',
        padding:'8px 12px',background:'#0A0C14',borderRadius:5,
        ...
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>setCollapsed({})}
            style={{padding:'3px 10px',background:'transparent',border:'1px solid #1A1D2E',
              borderRadius:3,color:'#64748B',fontSize:11,cursor:'pointer'}}>Expand all</button>
          <button onClick={()=>{
            const all={}
            ...
            style={{padding:'3px 10px',background:'transparent',border:'1px solid #1A1D2E',
              borderRadius:3,color:'#64748B',fontSize:11,cursor:'pointer'}}>Collapse all</button>
        </div>
      </div>
      {/* Tree */}
    ...
    <div style={{display:'flex',height:'100vh',background:C.bg,overflow:'hidden',fontSize:14}}>
      ...
      <div style={{width:252,background:C.sb,borderRight:`1px solid ${C.b1}`,display:'flex',flexDirection:'column',flexShrink:0}}>
        ...
        <div style={{padding:'15px 14px 12px',borderBottom:`1px solid ${C.b1}`}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
            <div style={{display:'flex',alignItems:'center',gap:9}}>
              <div style={{width:30,height:30,background:C.grad,borderRadius:5,display:'flex',alignItems:'center',justifyContent:'center',fontSize:15}}>🧠</div>
              <span style={{color:C.text,fontWeight:700,fontSize:15,letterSpacing:-0.3}}>VaultAI</span>
            </div>
            <button onClick={() => setShowGuide(true)}
              style={{background:'none',border:`1px solid ${C.b2}`,borderRadius:4,padding:'3px 8px',color:C.muted,fontSize:11.5,cursor:'pointer'}}>? Guide</button>
          </div>

          {/* Live presence */}
          <div style={{...eyebrow,fontSize:9.5,marginBottom:5}}>Online now</div>
        ...
        <div style={{padding:'10px 10px 4px'}}>
          <button onClick={() => setShowNew(true)}
            style={{width:'100%',padding:'8px 10px',background:'#FFFFFF12',border:'1px solid #FFFFFF33',borderRadius:5,
              color:'#E5E5E5',fontSize:13,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',gap:8}}>
          ...
          {projects.length === 0
            ...
            : projects.map(p => (
              <button key={p.id} onClick={() => { setProj(p); setTab('chat'); setNewMsgBanner(false) }}
                style={{width:'100%',textAlign:'left',padding:'9px 10px',borderRadius:5,border:'none',
                  background: proj?.id===p.id ? '#FFFFFF14' : 'transparent',cursor:'pointer',marginBottom:2,transition:'background .15s'}}
                ...
                <div style={{color: proj?.id===p.id ? '#EDEDED':'#94A3B8',fontSize:12.5,fontWeight:500,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                  📂 {p.name}
                </div>
        ...
        <div style={{padding:'8px',borderTop:`1px solid ${C.b1}`,display:'flex',gap:5}}>
          <button onClick={() => { setUsername(''); setApiKey(''); setProj(null) }}
            style={{flex:1,padding:'7px',background:C.panel,border:`1px solid ${C.b2}`,borderRadius:4,color:C.muted,fontSize:12,cursor:'pointer'}}>⇄ Switch</button>
          <button onClick={() => { setUsername(''); setProj(null); setMessages([]); setFiles([]); setContext('') }}
            style={{flex:1,padding:'7px',background:'#160A0A',border:'1px solid #2D1515',borderRadius:4,color:'#F87171',fontSize:12,cursor:'pointer'}}>✕ Logout</button>
        </div>
      </div>
        ...
        {!proj ? (
          ...
          <>
              ...
              <div style={{display:'flex',gap:5,flexShrink:0}}>
                ...
                {tab==='chat' && messages.length > 0 && <>
                  <button onClick={() => setShowThink(x=>!x)} title="Toggle thinking"
                    style={{...btn(false), background: showThink?'#0A0A0A':C.panel, color: showThink?'#E5E5E5':C.muted, border:`1px solid ${showThink?'#333333':C.b2}`}}>💭</button>
            ...
            {newMsgBanner && tab==='chat' && (
              <div style={{background:'#FFFFFF14',borderBottom:'1px solid #FFFFFF44',padding:'7px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
                <span style={{color:'#E5E5E5',fontSize:13}}>🔔 New messages from teammate</span>
                <button onClick={() => { setNewMsgBanner(false); isAtBottom.current=true; bottomRef.current?.scrollIntoView({behavior:'smooth'}) }}
                  style={{padding:'4px 12px',background:C.acc,border:'none',borderRadius:3,color:C.onAcc,fontSize:12,cursor:'pointer'}}>Jump ↓</button>
              </div>
            )}
                          ...
                          {dbProgress.map((r,i)=>{
                            ...
                            return (
                              <tr key={r.id} style={{borderBottom:`1px solid ${C.b1}`,background:i%2===0?'transparent':'#0A0C12'}}>
                                <td style={{padding:'6px 12px',color:'#EDEDED',fontFamily:"'JetBrains Mono',monospace",fontSize:11.5}}>{r.module}</td>
                        ...
                        <div key={r.id} style={{padding:'10px 14px',borderBottom:i<4?`1px solid ${C.b1}`:'none',display:'flex',gap:10,alignItems:'flex-start'}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:'flex',gap:8,marginBottom:3,flexWrap:'wrap'}}>
                              ...
                              <span style={{color:'#E5E5E5',fontSize:11}}>by {r.reviewer}</span>
                              <span style={{fontSize:11,padding:'1px 7px',borderRadius:6,background:r.severity==='critical'?'#5A1010':r.severity==='high'?'#3A1F0A':'#1A1D2E',color:r.severity==='critical'?'#F87171':r.severity==='high'?'#FB923C':'#64748B'}}>{r.severity}</span>
                            </div>
              ...
              <>
                    ...
                    <div key={m.id} style={{display:'flex',gap:10,justifyContent: m.role==='user'?'flex-end':'flex-start',alignItems:'flex-start'}}>
                      ...
                      <div style={{maxWidth:'76%'}}>
                        ...
                        <div style={{padding:'10px 13px',fontSize:13.5,lineHeight:1.65,
                          ...
                          color:        m.role==='user'?C.onAcc:'#CBD5E1',
                ...
                <div style={{padding:'10px 18px',borderTop:`1px solid ${C.b1}`,display:'flex',gap:8,flexShrink:0}}>
                  <textarea ref={inputRef} value={input}
                    ...
                    rows={1}
                    style={{flex:1,padding:'10px 13px',background:'#0F1118',border:`1px solid ${C.b2}`,borderRadius:6,
                  ...
                  <button onClick={sendMessage} disabled={!input.trim()||typing}
                    style={{padding:'10px 18px',background: input.trim()&&!typing?C.grad:'#151515',border:'none',
                      borderRadius:3,color: input.trim()&&!typing?C.onAcc:C.dim,fontSize:14,fontWeight:600,cursor: input.trim()&&!typing?'pointer':'not-allowed',
            ...
            {tab==='files' && (
              <div style={{flex:1,overflowY:'auto',padding:22}}>
                <div onClick={() => !uploading && fileRef.current?.click()}
                  style={{border:'2px dashed #1E2130',borderRadius:8,padding:'24px',textAlign:'center',
                    cursor: uploading?'wait':'pointer',marginBottom:18,transition:'all .2s'}}
                  onMouseEnter={e=>{ e.currentTarget.style.borderColor=C.acc; e.currentTarget.style.background='#FFFFFF08' }}
                  onMouseLeave={e=>{ e.currentTarget.style.borderColor='#262626'; e.currentTarget.style.background='transparent' }}>
                ...
                <div style={{marginBottom:14,display:'flex',alignItems:'flex-start',justifyContent:'space-between'}}>
                  ...
                  {!editCtx && (
                    <button onClick={() => { setCtxDraft(context); setEditCtx(true) }}
                      style={{padding:'7px 14px',background:'#FFFFFF14',border:'1px solid #FFFFFF44',borderRadius:4,color:'#E5E5E5',fontSize:13,fontWeight:500,cursor:'pointer',flexShrink:0,marginLeft:14}}>
                      ✏️ Edit
                    </button>
                  )}
                </div>
                  ...
                  <>
                    <textarea value={ctxDraft} onChange={e => setCtxDraft(e.target.value)}
                      ...
                      style={{width:'100%',minHeight:360,padding:'13px 15px',background:'#0F1118',border:`1px solid ${C.b2}`,
                        borderRadius:6,color:'#E2E8F0',fontSize:13.5,outline:'none',resize:'vertical',
                        fontFamily:'Inter,system-ui,sans-serif',lineHeight:1.75,boxSizing:'border-box'}}
                    />
                    <div style={{display:'flex',gap:10,marginTop:10}}>
                      <button onClick={() => setEditCtx(false)} style={{flex:1,padding:'10px',background:C.panel,border:`1px solid ${C.b2}`,borderRadius:5,color:C.muted,cursor:'pointer',fontSize:14}}>Cancel</button>
                      <button onClick={saveContext} style={{flex:2,padding:'10px',background:C.grad,border:'none',borderRadius:3,color:C.onAcc,fontWeight:600,cursor:'pointer',fontSize:14}}>💾 Save Context</button>
                    </div>
                  </>
                ) : (
                  <div style={{padding:'15px 17px',background:C.panel,border:`1px solid ${C.b2}`,borderRadius:6,minHeight:200}}>
                    {context
      ...
      {showNew && (
        <div style={{position:'fixed',inset:0,background:'#00000088',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200}}
          ...
          <div style={{background:'#0D0F17',border:`1px solid ${C.b2}`,borderRadius:10,padding:26,width:420,boxShadow:'0 30px 60px #00000070'}}>
            ...
            <input value={npName} onChange={e => setNpName(e.target.value)} onKeyDown={e => e.key==='Enter'&&createProject()} autoFocus
              ...
              style={{width:'100%',padding:'11px 13px',background:C.panel,border:`1px solid ${C.b2}`,borderRadius:5,color:C.text,fontSize:14,outline:'none',marginBottom:10,boxSizing:'border-box'}}/>
            <textarea value={npDesc} onChange={e => setNpDesc(e.target.value)}
              placeholder="Short description / intro for Claude (optional)…"
              rows={2}
              style={{width:'100%',padding:'11px 13px',background:C.panel,border:`1px solid ${C.b2}`,borderRadius:5,color:C.text,fontSize:14,outline:'none',resize:'none',boxSizing:'border-box',fontFamily:'Inter,system-ui,sans-serif'}}/>
            <div style={{display:'flex',gap:10,marginTop:14}}>
              <button onClick={() => setShowNew(false)} style={{flex:1,padding:'11px',background:C.panel,border:`1px solid ${C.b2}`,borderRadius:5,color:C.muted,cursor:'pointer',fontSize:14}}>Cancel</button>
              <button onClick={createProject} disabled={!npName.trim()}
                style={{flex:2,padding:'11px',background:npName.trim()?C.grad:'#151515',border:'none',borderRadius:3,color:npName.trim()?C.onAcc:C.dim,fontWeight:600,cursor:npName.trim()?'pointer':'not-allowed',fontSize:14}}>
                Create ✓
              </button>
            </div>
          </div>
      ...
      {viewFile && (
        <div style={{position:'fixed',inset:0,background:'#000000AA',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200}}
          ...
          <div style={{background:'#0D0F17',border:`1px solid ${C.b2}`,borderRadius:8,width:'80%',maxWidth:840,height:'80vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            <div style={{padding:'13px 18px',borderBottom:`1px solid ${C.b2}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <div style={{display:'flex',gap:8}}>
                <button onClick={() => downloadFile(viewFile)} style={{padding:'5px 13px',background:'#0A1929',border:'1px solid #1E3A5F',borderRadius:4,color:'#38BDF8',fontSize:13,cursor:'pointer'}}>↓ Download</button>
                <button onClick={() => setViewFile(null)} style={{padding:'5px 11px',background:'#160A0A',border:'1px solid #3A1010',borderRadius:4,color:'#F87171',fontSize:13,cursor:'pointer'}}>✕</button>
              </div>
            </div>
